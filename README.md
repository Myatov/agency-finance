...
При добавлении или изменении расхода бот отправляет сообщение в группу «Расходы». В `.env` укажите `TELEGRAM_BOT_TOKEN` и `TELEGRAM_EXPENSES_CHAT_ID`. Без них уведомления не отправляются.
