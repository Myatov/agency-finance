...
При добавлении или изменении расхода бот отправляет сообщение в группу «Расходы». В `.env` укажите:
- `TELEGRAM_BOT_TOKEN` — токен бота (например @seo_performance_manager_bot)
- `TELEGRAM_EXPENSES_CHAT_ID` — ID чата группы (например `-1001755796924`)

Без этих переменных уведомления не отправляются, раздел «Расходы» работает как раньше.
