...
При добавлении или изменении расхода бот шлёт уведомление в группу «Расходы» (`TELEGRAM_EXPENSES_CHAT_ID`) и дублирует в личный чат (`TELEGRAM_PERSONAL_CHAT_ID`, по умолчанию 135962813). В `.env`: `TELEGRAM_BOT_TOKEN` обязателен; для группы — `TELEGRAM_EXPENSES_CHAT_ID`; для личного дубля — `TELEGRAM_PERSONAL_CHAT_ID` (не задан = 135962813). Чтобы бот мог писать вам в личку, нажмите ему в Telegram «Старт» (/start). **Если сообщения не приходят:** откройте в браузере (под своей учёткой) `/api/telegram/test` — там будет видно, в какие чаты отправлено и успех/ошибка; на сервере смотрите логи: `pm2 logs agency-finance` — там текст ошибки от Telegram (например «Chat not found», «Forbidden: bot was blocked»).
