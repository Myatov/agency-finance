...
**Telegram (уведомления в разделе «Расходы»):**  
При добавлении или изменении расхода бот шлёт уведомление только в группу «Расходы». В `.env`: `TELEGRAM_BOT_TOKEN` и `TELEGRAM_EXPENSES_CHAT_ID` (ID группы).

**Если сообщения не приходят:** откройте (под своей учёткой) `https://ваш-домен/api/telegram/test` — там видно, задан ли токен и результат отправки; логи сервера: `pm2 logs agency-finance` (текст ошибки от Telegram).

**Локально приходят, с сервера — нет:** на сервере в `.env` нет переменных Telegram. Если `.env` на сервер попадает из секрета GitHub (при деплое): заново закодируй **текущий** `.env` (в нём уже есть `TELEGRAM_BOT_TOKEN` и при необходимости `TELEGRAM_EXPENSES_CHAT_ID`) в base64 и обнови секрет **ENV_FILE_CONTENT** в репозитории (Settings → Secrets → Actions). Затем сделай пустой коммит и пуш или перезапусти деплой — при следующем деплое на сервер запишется новый `.env` с Telegram. Альтернатива: зайти на сервер по SSH и вручную добавить в `/var/www/agency-finance/.env` строки `TELEGRAM_BOT_TOKEN=...` и `TELEGRAM_EXPENSES_CHAT_ID=...`, затем `pm2 restart agency-finance --update-env`.

**Личный кабинет клиента (ссылка для входа):**  
Чтобы ссылка в разделе «Клиенты → Личный кабинет» формировалась с вашим доменом (а не localhost), задайте в `.env` на сервере: `NEXT_PUBLIC_APP_URL=https://a.seo-performance.ru` (или ваш домен). Без этой переменной используется заголовок Host запроса.

**Файл .env на сервере:**  
При деплое через GitHub Actions файл `.env` на сервере можно обновлять автоматически из секрета. Один раз добавь в репозитории секрет **ENV_FILE_CONTENT** — значение: содержимое твоего `.env`, закодированное в base64 (подробно: **docs/GITHUB_ACTIONS_SETUP.md**). Тогда при каждом деплое этот `.env` будет записываться на VPS.
