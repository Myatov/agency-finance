...
export default function ReportsList() {
  const [activeTab, setActiveTab] = useState<'incomes' | 'expenses' | 'employees' | 'servicesWithoutPeriods'>('incomes');
  ...
  const handleExportCSV = () => {
    if (activeTab === 'incomes' && incomesData && incomesData.incomes) {
      ...
      const rows = servicesWithoutPeriodsData.services.map((s: any) => ({
        ...
        accountManager: s.site?.accountManager?.fullName ?? '',
      }));
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        {(incomesData || expensesData || employeesData || servicesWithoutPeriodsData) && (
          <button
