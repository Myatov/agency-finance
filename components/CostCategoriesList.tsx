...
export default function CostCategoriesList() {
  ...
  const handleDelete = async (item: CostCategory) => {
    if (!confirm(`Удалить категорию «${item.name}»?`)) return;
    const res = await fetch(`/api/cost-categories/${item.id}`, { method: 'DELETE' });
  ...
  const handleDrop = async (e: React.DragEvent, targetId: string) => {
    ...
    setItems(next);
    setDraggedItem(null);
    const res = await fetch('/api/cost-categories/reorder', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ categoryIds: next.map((i) => i.id) }),
    });
    if (!res.ok) {
      fetchItems();
      const data = await res.json();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    const url = editingItem ? `/api/cost-categories/${editingItem.id}` : '/api/cost-categories';
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            <h2 className="text-xl font-bold mb-4">{editingItem ? 'Редактировать категорию' : 'Добавить категорию'}</h2>
