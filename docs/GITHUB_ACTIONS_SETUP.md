...
После этого при каждом деплое на сервер будет записываться актуальный `.env`. Если секрет не задан — существующий `.env` на сервере не трогаем. **Если добавил в локальный `.env` новые переменные (например Telegram)** — заново выполни команду base64 для текущего `.env`, обнови значение секрета ENV_FILE_CONTENT в GitHub и при следующем деплое сервер получит новый `.env`.
